...
router.post('/login', async (req, res, next) => {
  try {
    ...
    // Check if user is active FIRST (before password - security)
    if (!user.isActive) {
      return res.status(403).json({ message: 'Tu cuenta está inactiva. Ponte en contacto con el administrador.' });
    }
    ...
    // Check if user is verified (only for new registrations)
    if (!user.isVerified) {
