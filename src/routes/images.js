...
router.post('/refresh-dantia/:id', authenticate, async function(req, res, next) {
  try {
    ...
    const dantiaArticle = await dantiaService.validateArticleCode(image.articleCode);
...
router.get('/filters', authenticate, async function(req, res, next) {
  try {
    let filterOptions = await FilterOptions.findOne().lean();
    
    if (!filterOptions || isCacheOld(filterOptions.lastUpdated)) {
      var dantiaFilters = await dantiaService.getFilterOptions();
      
      if (!filterOptions) {
        filterOptions = new FilterOptions();
      }
      
      filterOptions.familias = dantiaFilters.familias || [];
      filterOptions.macetas = dantiaFilters.macetas || [];
      filterOptions.alturas = dantiaFilters.alturas || [];
      filterOptions.lastUpdated = new Date();
      await filterOptions.save();
    }
    
    res.json({
      familias: filterOptions.familias,
      macetas: filterOptions.macetas,
      alturas: filterOptions.alturas
    });
  } catch (error) {
    next(error);
  }
...
router.post('/upload', authenticate, authorize('editor', 'admin'), upload.single('image'), async function(req, res, next) {
  try {
    ...
    const cloudinaryResult = await cloudinaryService.uploadImage(req.file.buffer, publicId);
...
router.put('/:id', authenticate, async function(req, res, next) {
  try {
    ...
    // Also update Dantia info with new code
    const dantiaArticle = await dantiaService.validateArticleCode(image.articleCode);
    if (dantiaArticle) {
      image.dantiaInfo = {
        CodigoArticulo: dantiaArticle.CodigoArticulo,
